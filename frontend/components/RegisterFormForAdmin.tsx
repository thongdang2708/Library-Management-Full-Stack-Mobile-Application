...
interface FormProps {
    isLogin: boolean,
    getAuthentication: (userInputs : AdminInputProps) => void
}

interface isValidProps {
    secretKey: undefined | boolean,
...
function RegisterFormForAdmin( {isLogin, getAuthentication} : FormProps) {
    ...
    let regexEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/
        ...
        let adminKeyIsValid = userInputs.secretKey.length > 0;
        let usernameIsValid = (userInputs.username.length >= 4 && userInputs.username.length <= 20);
        ...
        if (!usernameIsValid || !emailIsValid || !passwordIsValid || !confirmPasswordIsValid || !adminKeyIsValid) {
            
            setIsValid((valid) => {
                return {
                    secretKey: adminKeyIsValid,
        ...
        getAuthentication({
            secretKey: userInputs.secretKey,
  ...
  return (
    <View style={styles.form}>
        ...
        {(<Input name='Admin Key' textInputConfig={{
            placeholder: "Please enter your admin key to register as admin",
            autoCorrect: false,
            autoCapitalize: "none",
            value: inputs.secretKey,
            onChangeText: (text : string) => handleChangeInput("secretKey", text)
        }} isValid={isValid.secretKey === false} textInValid="Please check admin key. Admin key must be given and must not be blank!"
        />)}

        {(<Input name='Username' textInputConfig={{
        ...
        <ButtonForForm onPress={handleSubmit}>
            {isLogin === false ? "Register as an admin for the application" : "Log In"}
        </ButtonForForm>
...
export default RegisterFormForAdmin;